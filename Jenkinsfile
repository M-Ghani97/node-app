        ...
        stage ('Deploy') {
            steps{
                sshagent(credentials : ['ec2-deploy']) {
                    ...
                    sh "ssh -v ubuntu@ec2-54-90-29-225.compute-1.amazonaws.com 'docker stop $registry:$prevBUILD && docker rm $registry:$prevBUILD && docker pull $registry:$BUILD_NUMBER && docker run -d --name=node-app-$BUILD_NUMBER -p 3000:3000 $registry:$BUILD_NUMBER'"
                }
            }
        }
        stage('Cleaning Up') {
